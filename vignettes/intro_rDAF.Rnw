...
The \pkg{rDAF} package provides functions to support assimilation prototyping within the  \proglang{R} environment.
In the present version, only the core assimilation function {\tt assimilateLite} is provided, which conducts a global ensemble Kalman filter (EnKF).
A more developed version of \pkg{rDAF}, whose documentation is under preparation, includes  the function {\tt assimilate}, as well as functions which it depends on. 
This function includes additional features as localization and inflation, generation of autocorrelated noise fields to simulate model error, etc. Further, it is parallelised
via the package \pkg{rmpi}, a wrapper to MPI libraries. In the interim time that the documentation for {\tt assimilate} and the related functions is under preparation, please contact the author for (undocumented) requests. 

The function {\tt analyseUG} prepares the input for both {\tt assimilateLite} and {\tt assimilate}, and I have considered that the detailed 
description of some structured input needed for {\tt analyseUG} would be better placed in this additional document than within the specific function documentation. 
...
study.
\textit{Journal of Hydrology} 2015; 523:706--724.
...
\textit{J. Hydrol.\/} 2013; 495:252--266.
